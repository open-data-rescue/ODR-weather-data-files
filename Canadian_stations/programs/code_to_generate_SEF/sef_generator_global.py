    ...
    if cfg["fields"][t] is not None:
        ...
        for fields in cfg["fields"][t]:
            ...
            query = ('select a.observation_date, de1.value, de1.id, p.image_file_name, a.transcription_id, '
                     ...
                     'where a.page_id in (select id from pages where '
                     'title like \''+cfg["site"]["prefix"]+'%\') '
...
df_clean.to_sql(con=engine, name='data_clean', if_exists='append', chunksize=100, method='multi', index=False)
df_iso.to_sql(con=engine, name='data_iso', if_exists='append', chunksize=100, method='multi', index=False)
